...
const response = await queue.add(() => useApiCache(`/books/${route.params.book}`), { priority: 3 })
const book = ref(response && response.data ? response.data.book : {})
const pages = ref(response && response.data ? response.data.pages : [])
const resources = ref(response && response.data ? response.data.resources : [])

pages.value.forEach((page) => useCache(page.image, page.image_url))
...
if (!response || !response.data) {
  ...
  onMounted(() => {
    flipBook(
      ...
      book.value.height ? book.value.height : 600
    )
  })
}
</script>
...
<template>
  ...
  >
    <div v-if="book.isbn" class="mb-6">
      <label>ISBN</label>
      <div class="mt-2 border py-3 px-4 rounded-md border-gray-300" v-html="book.isbn" />
    </div>
  ...
  <div
    class="min-h-screen h-full flex flex-col bg-fixed bg-center bg-no-repeat bg-cover bg-gray-50 text-gray-700"
  >
    <header class="grid grid-cols-3 bg-white shadow p-2 items-center">
      <div>
        <LayoutLogo class="!max-h-20 !mx-none" />
      </div>
      <div>
        ...
        <div class="flex items-center justify-center space-x-2">
          <button
            type="button"
            class="flex items-center justify-center w-10 h-10 text-white bg-black rounded-full outline-none prev focus:outline-none hover:opacity-70"
          >
            <IconMdiChevronLeft />
          </button>
          <span>Page</span>
          <input
            value="1"
            type="number"
            class="w-20 px-3 py-2 text-center border rounded page-input"
          />
          <span>of</span>
          <span class="total-pages">{{ pages.length }}</span>
          <button
            type="button"
            class="flex items-center justify-center w-10 h-10 text-white bg-black rounded-full outline-none next focus:outline-none hover:opacity-70"
          >
            <IconMdiChevronRight />
          </button>
        </div>
      ...
      <div class="flex items-center justify-end space-x-2">
        <button
          v-if="book.isbn || book.description"
          type="button"
          class="flex items-center justify-center w-10 h-10 text-white bg-black rounded-full outline-none focus:outline-none hover:opacity-70"
          @click="modalInformations = true"
        >
          <IconMdiHelp />
        </button>
        <button
          v-if="online && resources.length"
          type="button"
          class="flex items-center justify-center w-10 h-10 text-white bg-black rounded-full outline-none focus:outline-none hover:opacity-70"
          @click="modalResources = true"
        >
        ...
        <button
          type="button"
          class="flex items-center justify-center w-10 h-10 text-white bg-black rounded-full outline-none zoom-in focus:outline-none hover:opacity-70"
        >
          <IconBiZoomIn />
        </button>
        <button
          type="button"
          class="flex items-center justify-center w-10 h-10 text-white bg-black rounded-full outline-none zoom-out focus:outline-none hover:opacity-70"
        >
          <IconBiZoomOut />
        </button>
        <router-link
  ...
  .content {
    ...
    .content-icon {
      ...
      --un-ring-shadow: var(--un-ring-inset) 0 0 0
        calc(var(--un-ring-width) + var(--un-ring-offset-width)) var(--un-ring-color);
      box-shadow: var(--un-ring-offset-shadow), var(--un-ring-shadow), var(--un-shadow);
    }
  }
